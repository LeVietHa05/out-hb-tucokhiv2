...
export default function ImageDisplay() {
  const { data, error } = useSWR('/api/image', fetcher, { refreshInterval: 2000 });
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border h-full flex flex-col">
      <div className="p-4 border-b">
        ...
        <p className="text-sm text-gray-500">Last updated: {new Date(data.timestamp).toLocaleTimeString()}</p>
      </div>
      ...
      {data.metadata && (
        <div className="p-4 border-t bg-gray-50">
          ...
          <div className="flex flex-wrap gap-2">
            {data.metadata.detected_tools?.map((tool: string, index: number) => (
              <span
                key={index}
                className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800"
              >
                {tool} ({Math.round((data.metadata.confidence_scores?.[index] || 0) * 100)}%)
              </span>
